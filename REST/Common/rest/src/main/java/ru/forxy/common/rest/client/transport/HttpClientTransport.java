...
import ru.forxy.common.rest.client.retry.IRetryPolicy;
import ru.forxy.common.rest.client.retry.RetryExecutor;
import ru.forxy.common.rest.client.transport.support.RepeatableEntityCreatingResponseInterceptor;
...
public class HttpClientTransport implements ITransport {
        ...
        if (HttpGet.METHOD_NAME.equals(method)) {
            ...
            if (entity != null) {
                try {
                    ...
                    throw new ClientException(null, e, RESTCommonEventLogId.UnexpectedException, e.getMessage());
                }
            }
        } else if (HttpDelete.METHOD_NAME.equals(method)) {
            request = new HttpDelete(url);
        } else {
            throw new ClientException(null, RESTCommonEventLogId.UnexpectedException,
    ...
    private static <R, E> Response<R, E> executeRequest(final HttpClient httpClient,
        ...
        try {
            ...
            throw new ClientException(null, e, RESTCommonEventLogId.SSLConnectivityException,
                    "[" + request.toString() + "]: " + e.getMessage());
        } catch (final java.net.SocketTimeoutException e) {
            throw new ClientException(null, e, RESTCommonEventLogId.SocketTimeoutException,
                    "[" + request.toString() + "]: " + e.getMessage());
        } catch (final org.apache.http.conn.HttpHostConnectException e) {
            throw new ClientException(null, e, RESTCommonEventLogId.ServiceUnavailableException,
                    "[" + request.toString() + "]: " + e.getMessage());
    ...
    private class SimpleRetryExecutor<R, E> extends RetryExecutor<Response<R, E>, ClientException> {
        ...
        protected ClientException postProcessLastException(final Exception lastException) {
            if (lastException instanceof ClientException) {
                ...
                return new ClientException(null, lastException, RESTCommonEventLogId.UnexpectedException,
